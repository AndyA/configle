 ...
 *          - A smartstring that will be expanded and used to search for config files.
 *
...
Configle.prototype = {
  ...
  options: function() {
    if (this.myOptions !== undefined) {
      throw new errors.ConfigleCoreError("Can't call options() after getOptions() or load()");
    }
  ...
  addConfig: function(config, fromDir) {
    if (this.config !== undefined) {
      throw new errors.ConfigleCoreError("Can't add config after get() or load()");
    }
  ...
  loadFile: function(name, loader) {
    ...
    if (loader === undefined || !_.isFunction(loader)) {
      ...
      if (loader === undefined) {
        throw new errors.ConfigleCoreError("Can't find loader for " + name);
      }
    }
  ...
  load: function() {
    ...
    if (!opt.hasOwnProperty("baseName")) {
      throw new errors.ConfigleCoreError("Required option baseName is missing");
    }

    var numFound = 0;
    var namer = new MultiString(opt.baseName);
    var dir = fs.realpathSync(opt.startDir);
    var loaderNames = _.keys(opt.loaders)
      .sort();
