...
Configle.prototype = {
  ...
  options: function() {
    if (this.myOptions !== undefined) {
      throw new errors.ConfigleCoreError(
        "Can't call options() after getOptions() or load()");
    }
  ...
  addConfig: function(config, fromDir) {
    if (this.config !== undefined) {
      throw new errors.ConfigleCoreError(
        "Can't add config after get() or load()");
    }
  ...
  loadFile: function(name, loader) {
    ...
    return this.addConfig(loader(data.toString()), dir);
  },
  ...
  load: function() {
    ...
    if (!opt.hasOwnProperty("baseName")) {
      throw new errors.ConfigleCoreError(
        "Required option baseName is missing");
    }
